...
export const getTripsByTravelStyle = async () => {
  ...
  const travelStyleCounts = trips.documents.reduce(
    (
      acc: { [key: string]: { count: number; firstTripName: string } },
      trip: Document
    ) => {
      const tripDetail = parseTripData(trip.tripDetails);

      if (tripDetail && tripDetail.interests) {
        const interest = tripDetail.interests;
        if (!acc[interest]) {
          acc[interest] = {
            count: 0,
            firstTripName: tripDetail.name || "Unknown Trip",
          };
        }
        acc[interest].count += 1;
      }
      return acc;
    },
    {}
  ...
  return Object.entries(travelStyleCounts).map(([interest, data]) => ({
    interest,
    count: data.count,
    firstTripName: data.firstTripName,
  }));
};
